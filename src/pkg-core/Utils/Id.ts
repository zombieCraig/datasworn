...
type ExtractNonCollectableTypeId<T extends NonCollectable> = Split<
	T,
	CONST.PrefixSep
> extends [infer TypeId extends TypeId.NonCollectable, ...string[]]
	? TypeId
	: never
type ExtractCollectableTypeId<T extends Collectable> = Split<
	T,
	CONST.PrefixSep
> extends [infer TypeId extends TypeId.Collectable, ...string[]]
	? TypeId
	: never
type ExtractCollectionTypeId<T extends Collection> = Split<
	T,
	CONST.PrefixSep
> extends [infer TypeId extends TypeId.Collection, ...string[]]
	? TypeId
	: never
...
type ExtractPrimaryPath<T extends Primary> = Split<
	ExtractPath<T>,
	CONST.TypeSep
> extends [infer U extends string, ...string[]]
	? U
	: Split<ExtractPath<T>, CONST.TypeSep> extends [infer U extends string]
		? U
		: never

export type ExtractRulesPackage<T extends Primary> = Split<
...
export type ExtractKey<T extends Primary> = Split<T> extends [
	...
	? Key
	: Split<T> extends [TypeId.Primary, RulesPackageId, infer Key extends string]
		? Key
		: never
